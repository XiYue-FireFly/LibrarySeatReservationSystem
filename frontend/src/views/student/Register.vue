<template>
  <div class="login-container">
    <div class="login-box">
      <GlassCard title="学生注册">
        <form @submit.prevent="handleRegister" class="login-form">
          <div class="form-group">
            <label>学号</label>
            <input v-model="form.account" @blur="checkAccount" type="text" class="glass-input" placeholder="请输入8-12位学号" />
          ...
          <div class="form-group mt-4">
            <label>用户名 (可选)</label>
            <input v-model="form.userName" type="text" class="glass-input" placeholder="请输入用户名" />
          </div>
          <div class="form-group mt-4">
            <label>密码</label>
          ...
          <div class="form-group mt-4">
            ...
            <div class="avatar-preview mt-2">
              <span>预览：</span>
              <img :src="form.avatar || 'https://api.dicebear.com/7.x/avataaars/svg?seed=preview'" class="preview-img" />
            </div>
          </div>
          ...
          <div class="admin-link mt-4 text-center">
            <router-link to="/student/login">已有账号？返回登录</router-link>
          </div>
        </form>
      </GlassCard>
    </div>
...
const checkAccount = async () => {
  if (!form.value.account || form.value.account.length < 8) return;
  try {
    const res = await request.get(`/common/check-account?account=${form.value.account}&type=STUDENT`)
    if (res.code === 200 && res.data.isExist) {
      accountError.value = '该学号已被注册'
    } else {
      accountError.value = ''
    }
  } catch (e) {
...
const handleRegister = async () => {
  ...
  if (!form.value.account || !/^\d{8,12}$/.test(form.value.account)) {
    errorMsg.value = '学号必须是8-12位纯数字'
    return
  }
